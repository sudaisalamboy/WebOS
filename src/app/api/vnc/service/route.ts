...
async function startService(service: string): Promise<{ ok: boolean; error?: string }> {
  ...
  switch (service) {
    ...
    case 'x11vnc': {
      ...
      // Start x11vnc watchdog via setsid+nohup so it fully detaches into its own
      // session and survives the request lifecycle (plain detached:true was dying).
      return new Promise((resolve) => {
        const logFd = fs.openSync(`${LOG_DIR}/x11vnc.log`, 'a')
        const p = spawn('setsid', [
          'python3', '-u', '/home/z/my-project/scripts/x11vnc-watchdog.py',
        ], {
          cwd: '/home/z/my-project',
          env: { ...process.env },
        ...
        p.unref()
        // Wait up to 8s for x11vnc to bind port 5900
        let waited = 0
        const interval = setInterval(async () => {
          waited += 1000
          if (await checkPort(VNC_PORT)) {
            clearInterval(interval)
            resolve({ ok: true })
          } else if (waited >= 8000) {
            clearInterval(interval)
            resolve({ ok: false, error: 'x11vnc did not bind port 5900 within 8s — check log' })
          }
        }, 1000)
      })
    }
    case 'websockify': {
      ...
      return new Promise((resolve) => {
        // Use setsid so websockify fully detaches into its own session
        // (plain detached:true was getting killed when the request ended).
        const logFd = fs.openSync(`${LOG_DIR}/websockify.log`, 'a')
        const p = spawn('setsid', [
    ...
    case 'all': {
      // Start all 4 in order. Each step short-circuits if already running,
      // and uses the fixed setsid-based spawn so they survive the request.
      ...
      return { ok: Object.values(results).every(v => v), results }
    }
    default:
